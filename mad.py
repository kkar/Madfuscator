...
replacements = {
    ...
    '[valloc]': ''.join(random.choice(charset) for x in range(random.randint(3, 40))),
    '[cThread]': ''.join(random.choice(charset) for x in range(random.randint(3, 40)))
}
...
types = ''.join(random.choice(charset) for x in range(random.randint(3, 40)))

# DISCLAIMER: This is a calculator shellcode generated with Metasploit Framework.
...
with open('new_template.py', 'r') as template:
...
source = source.replace('[types]', types)
...
with open('fud.py', 'w') as final:
    ...
    final.write('import ctypes as %s\n\n' % types)
